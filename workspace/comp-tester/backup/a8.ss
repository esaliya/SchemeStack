    ...
    (lambda (body param*)
      ...
      (define (Pred ap)
        (lambda (pred)
          (match pred
            ...
            [(begin ,[(Effect ap) -> ef*] ... ,[pred]) (make-begin `(,ef* ... ,pred))]
      ...
      (define (Effect ap)
        (lambda (ef)
          (match ef
            [(nop) '(nop)]
            [(if ,[(Pred ap) -> pred] ,[conseq] ,[alter]) `(if ,pred ,conseq ,alter)]
            [(begin ,[ef*] ... ,[ef]) (make-begin `(,ef* ... ,ef))]
               ...
               `(return-point ,rp-label
                              ,(make-begin
                                 `((set! ,(reverse nt-arg-loc*) ,(reverse rand*)) ...
                                   ...
                                   (set! ,allocation-pointer-register ,ap)
      ...
      (define Tail
        (lambda (rp ap)
          (lambda (tail)
            (match tail
              [(begin ,[(Effect ap) -> ef*] ... ,[tail])
               (make-begin `(,ef* ... ,tail))]
              [(if ,[(Pred ap) -> pred] ,[conseq] ,[alter])
               `(if ,pred ,conseq ,alter)]
              [(alloc ,[Triv -> n]) 
               (make-begin
                 `((set! ,return-value-register ,ap)
                   (set! ,ap (+ ,ap ,n))
                   (set! ,allocation-pointer-register ,ap)
              ...
              [(,binop ,[Triv -> triv1] ,[Triv -> triv2]) (guard (binop? binop))
               (make-begin
                `((set! ,return-value-register (,binop ,triv1 ,triv2))
                  ;; set allocation-pointer-register to the value of ap
               ...
               (let ([arg-loc* (param-locations triv*)])
                 (make-begin
                  `((set! ,(reverse arg-loc*) ,(reverse triv*)) ...
                    ...
                    ;; set allocation-pointer-register to the value of ap
                    (set! ,allocation-pointer-register ,ap)
                    ;; mark allocation-pointer-register as live at the call
      ...
      (match body
        [(locals (,local* ...) ,tail)
         (let ([rp (unique-name 'rp)]
               [ap (unique-name 'ap)] ;; create a uniqe variable for allocation-pointer
               [param-loc* (param-locations param*)])
           (let ([tail ((Tail rp ap) tail)])
             ;; add ap to the set of locals
             `(locals (,local* ... ,rp ,ap ,param* ... ,newframe** ... ...)
                (new-frames ,newframe**
                  ,(make-begin
                    `((set! ,rp ,return-address-register)
                      ;; set ap to the value of allocation-pointer-register
                      (set! ,ap ,allocation-pointer-register)
                      (set! ,param* ,param-loc*) ...
                      ,tail))))))]
        [,x (error who "invalid Body ~s" x)])))
  (lambda (x)
...
;
; Output Language: 
;   Note. Only change is made to the Body form.
;
;   Program	-->	(letrec ([label (lambda (uvar*) Body)]*) Body)
;   Body --> (locals (uvar*)
;              (new-frames (frame*)
