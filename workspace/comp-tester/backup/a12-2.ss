...
(define-who introduce-procedure-primitives
  ...
  (define (Lambda lamb)
    (match lamb
      ...
      ;; (the second is that procedure should be well-known (see C) 
             ...
             (if (memq rator f*)
                 (let ([tmp (unique-name 'tmp)])
                   `(let ([,tmp (procedure-ref ,cp (quote ,(set-idx rator f*)))])
                      ((procedure-code ,tmp) ,tmp ,ex* ...))) ; here rator = ratorcp
                 `((procedure-code ,rator) ,ratorcp ,ex* ...)) ; here too rator = ratorcp
             `(,((Expr cp f*) rator) ,((Expr cp f*) ratorcp) ,ex* ...))] 
        [,x (error who "invalid Expr ~s" x)])))
  (lambda (x)
    (match x
      [,[(Expr 'dummy.cp '()) -> ex] ex])))
