...
(define-label value-of
  ; remember we had expr env and k
  (union-case expr exp
    [(const n) (begin
                 (set! v n)
                 (set! pc apply-k))]
  ...
  (begin
    (begin
      (set! expr (exp_mult
                  ...
                  (exp_letcc
                   (exp_mult
                    (exp_const 2)
                    (exp_throw (exp_const 4) (exp_var 0))))))
      (set! env (envr_empty))
      (set! pc value-of)
    ...
    (begin
      (set! expr (exp_mult
                  ...
                  (exp_letcc
                   (exp_mult
                    (exp_letcc (exp_throw (exp_const 3) (exp_var 1)))
                    (exp_const 5)))))
      (set! env (envr_empty))
      (set! pc value-of)
      (mount-trampoline kt_empty_k k pc)
    ...
    (begin
      (set! expr (exp_app (exp_app (exp_app (exp_lambda (exp_lambda (exp_lambda (exp_mult (exp_var 2) (exp_var 0))))) (exp_const 5)) (exp_const 6)) (exp_const 7)))
      (set! env (envr_empty))
      (set! pc value-of)
      (mount-trampoline kt_empty_k k pc)
      (printf "value-of -> ((lambda (a b c) (* a c)) 5 6 7): ~d\n" v))
    (begin
      (set! expr (exp_const 10))
      (set! env (envr_empty))
      (set! pc value-of)
      (mount-trampoline kt_empty_k k pc)
      (printf "value-of -> 10: ~d\n" v))
    (begin
      (set! expr (exp_mult (exp_const 2) (exp_const 3)))
      (set! env (envr_empty))
      (set! pc value-of)
      (mount-trampoline kt_empty_k k pc)
      (printf "value-of -> (* 2 3): ~d\n" v))
    (begin
      (set! expr (exp_if (exp_zero (exp_const 0)) (exp_const 1) (exp_const 0)))
      (set! env (envr_empty))
      (set! pc value-of)
      (mount-trampoline kt_empty_k k pc)
      (printf "value-of -> (if (zero? 0) 1 0): ~d\n" v))
    (begin
      (set! expr (exp_if (exp_zero (exp_const 1)) (exp_const 0) (exp_const 1)))
      (set! env (envr_empty))
      (set! pc value-of)
      (mount-trampoline kt_empty_k k pc)
      (printf "value-of -> (if (zero? 1) 0 1): ~d\n" v))
    (begin
      (set! expr (exp_sub1 (exp_const 7)))
      (set! env (envr_empty))
      (set! pc value-of)
      (mount-trampoline kt_empty_k k pc)
