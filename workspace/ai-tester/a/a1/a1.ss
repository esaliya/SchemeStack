...
;;              (state-print current)
              (if (hashtable-contains? visited current-key)
                  (try attempts)
              ...
              (let ([child (state-copy state)])
                ...
                (let ([key (state-key child)])
                  (if (not (hashtable-contains? visited key))
                      (begin
                        (state-p-set! child state) ;; Set parent
                        (state-depth-set! child (add1 (state-depth state))) ;; Set depth
                        (state-move-set! child (cdr (assq (car moves) opp-move-names))) ;; Set move
...
(define h2
  (lambda (cv gv)
    (let loop ([i 0] [count 0])
      (if (eq? i (vector-length gv))
          count
          (let ([ctx (div i 3)] [cty (mod i 3)] [j (vector-idxq gv (vector-ref cv i))])
            (let ([gtx (div j 3)] [gty (mod j 3)])
...
;; Possible moves for the blank. The association is of the form
;; (blank-idx . (move ...))
